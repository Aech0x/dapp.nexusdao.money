...
const TierRadioGroup = React.forwardRef<{
  tier: number
  name: string
  usePresale: boolean
  ...
  const [mintWithPresale, setMintWithPresale] = useState(false)

  useImperativeHandle(
    ref,
    () => {
      return { tier: selectedTier, name: mintName, usePresale: mintWithPresale }
    },
    [selectedTier, mintName, mintWithPresale]
  )
  ...
  return (
    <React.Fragment>
      ...
      <div className="mt-2">
        <label htmlFor="mintFromInvested" className="inline-flex items-center">
          <input
            type="checkbox"
            name="mintFromInvested"
            id="mintFromInvested"
            checked={mintWithPresale}
            onChange={(e) => {
              setMintWithPresale(e.target.checked)
            }}
          />
          <span className="text-sm text-gray-500 ml-2">
            Use presale balance for minting
          </span>
        </label>
      </div>
    </React.Fragment>
                    ...
                    if (!isStakingApproved) {
                      setModalData({
                        ...
                        buttonFunc: () => {
                          ...
                          dispatch({ type: "nodes/approveNFT" })
                        }
                      })
